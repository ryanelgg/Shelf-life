...
function extractDateOffset(text: string, today: Date): { offset: number; matched: string } | null {
  ...
  // Trailing (?![-\s]\w) is essential too: \b treats both a hyphen and a space
  // as a boundary, so "sun-dried tomatoes" AND "sun dried tomatoes" would
  // otherwise match "sun" (→ Sunday), strip it, and leave a corrupted "Dried
  // Tomatoes" with a bogus expiry. A weekday followed by another word is part of
  // the item name, not a date. A trailing weekday ("milk friday", end of input)
  // still parses because there is no following word.
