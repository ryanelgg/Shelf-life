...
export async function validateCommunityProduct(
  ...
  try {
    // Sends the signed-in user's own session token (not the public anon key)
    // so the function can verify the caller and enforce its daily rate limit —
    // matches every other AI edge function. Guests (no session) fail open,
    // same as any other validation failure, without ever reaching the network.
    const res = await fetch(validateUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...(await getAiAuthHeaders(supabaseAnonKey)) },
...
export async function readProductPhoto(base64Image: string): Promise<PhotoReadResult> {
  // Sends the signed-in user's own session token (not the public anon key) so
  // the function can verify the caller and enforce its daily rate limit —
  // matches every other AI edge function.
